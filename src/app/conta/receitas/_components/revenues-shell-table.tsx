...
import { deleteRevenueBatchAction } from '@/actions/revenue/delete-batch-revenue-action'
import { deleteRevenueAction } from '@/actions/revenue/delete-revenue-action'
import { updateRevenueAction } from '@/actions/revenue/update-revenue-action'
...
export function RevenuesTableShell({
  ...
  const [selectedRevenue, setSelectedRevenue] =
    useState<RevenueWithCounts | null>(null)
  ...
  const { execute, isPending } = useStateAction(updateRevenueAction, {
    onSuccess: () => {
  ...
  const { execute: executeDeleteBatch, isPending: isPendingDeleteBatch } =
    useStateAction(deleteRevenueBatchAction, {
      onSuccess: () => {
        toast.success('Cobranças deletadas com sucesso')
        setConfirmBatchDelete(false)
      },
      onError: ({ error }) => {
        toast.error('Erro ao deletar cobranças', {
          description: error.serverError,
        })
        setConfirmBatchDelete(false)
      },
  ...
  const formMethods = useForm<UpdateRevenueSchemaType>({
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    resolver: zodResolver(updateRevenueSchema) as any,
  })
  ...
  const handleDeleteRevenue = useCallback(() => {
    if (selectedRevenue) {
      executeDelete({ id: selectedRevenue.id })
    }
  }, [executeDelete, selectedRevenue])
  ...
  const columns = useMemo<ColumnDef<RevenueWithCounts, unknown>[]>(
    () => [
      ...
      },
      {
        ...
        header: ({ column }) => (
          <DataTableColumnHeader column={column} title="Tipo de Recebimento" />
        ),
        cell: ({ row }) => {
              ...
              <DropdownMenuItem asChild>
                <Button
                  ...
                  onClick={() => {
                    ...
                    setSelectedRevenue(row.original)
  ...
  return (
    <>
      ...
      <ConfirmDialog
        ...
        open={!!selectedRevenue && isDeleteItem}
        onCancel={() => {
          setSelectedRevenue(null)
